...
function handleAttendanceChange(e) {
  if (e.target.value === 'yes') {
    ...
    // Remove required attribute since we allow 0 guests
...
function validateForm(formData) {
  ...
  if (formData.attendance === 'yes') {
    ...
    if (totalGuests > 5) {  // Removed the lower bound check
      throw new Error("Total number of guests (adults + kids) cannot exceed 5");
    }
  }
}
...
document.getElementById("guestsAdult").addEventListener("input", (e) => {
  ...
  if (e.target.value === '') return; // Allow empty value
